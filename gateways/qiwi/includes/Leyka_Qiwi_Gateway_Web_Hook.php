...
class Leyka_Qiwi_Gateway_Web_Hook {
    ...
    public static function hook() {
        ...
        $signature = empty($_SERVER['HTTP_X_API_SIGNATURE_SHA256']) ? false : $_SERVER['HTTP_X_API_SIGNATURE_SHA256'];

        $bill_id = $data_decode['payment']['billId'];
        $donation_id = absint($bill_id); // = Leyka_Qiwi_Gateway_Helper::get_payment_id_by_response_data($bill_id);
        $donation = Leyka_Donations::get_instance()->get($donation_id);

        if( // Signature is sent, but not correct
            $signature
            && !Leyka_Qiwi_Gateway_Web_Hook_Verification::check_notification_signature(
                $signature,
                $data_decode,
        ...
        $status = $data_decode['payment']['status']['value'];
