...
class Leyka_Dashamail_Extension extends Leyka_Extension {
                ...
                'section' => [
                    ...
                    'options' => [
                        ...
                        $this->_id.'_donor_fields' => [
                            ...
                            'description' => 'Существует ограничение на количество дополнительных полей - в выбранную адресную базу может быть добавлено не более '.($this->_get_fields_amount() - 1).'. Поля "Имя" и "Email" являются основными и по умолчанию будут записаны в адресную базу',
                            'is_html' => true,
    ...
    protected function _get_fields_amount() {

        if (isset($_POST['leyka_dashamail_api_key'])) {
            $api_key = $_POST['leyka_dashamail_api_key'];
        } else {
            $api_key = leyka_options()->get_option_value($this->_id.'_api_key');
        }

        if (isset($_POST['leyka_dashamail_lists'])) {
            $list_id = $_POST['leyka_dashamail_lists'];
        } else {
            $list_id = leyka_options()->get_option_value($this->_id.'_lists');
        }

        $apiClass = new \Dashamail\ApiWrapper\DashamailApi($api_key);

        $json_lists = $apiClass->getLists(['list_id'=>$list_id]);
        $full_lists = json_decode($json_lists, true)['response']['data'];

        $fields_amount = 0;
        foreach($full_lists[0] as $key => $value) {
            if (str_contains(strval($key), 'merge_')) {
                $fields_amount++;
            }
        }

        if ($fields_amount > 10) {
            $result = $fields_amount;
        } else {
            $result = 10;
        }
    ...
    public function _add_donor_to_dashamail_list($donation_id, $old_status, $new_status) {
        ...
        if ($old_status == 'funded' || $new_status == 'funded' || $old_status !== 'funded' || $new_status !== 'funded') {
            ...
            $arr_donor_fields = $this->_get_donor_fields();
            $donation_additional_fields = [];

            foreach($arr_donor_fields as $field_name) {
                if ($field_name === 'name') {
                    $donor_fields[$field_name] = $donation->donor_name;
                } 
                $donation_additional_fields = array_merge($donation_additional_fields, $donation->additional_fields);
            }
        }
            ...
            if ($donation_additional_fields !== []) {
                ...
                foreach ($donation_additional_fields as $field=>$value) { 
                        ...
                        if (!in_array($field, $curr_list_merges) && !in_array($value, $curr_list_merges_titles) && $i<=$this->_get_fields_amount()) {
