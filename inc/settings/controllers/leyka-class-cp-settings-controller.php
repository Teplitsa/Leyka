...
class Leyka_Cp_Wizard_Settings_Controller extends Leyka_Wizard_Settings_Controller {
    ...
    protected function _set_stages() {
        ...
        $section = new Leyka_Settings_Section('send_documents',  $stage->id, __('Sending the documents', 'leyka'), array('next_label' => __('Send the email', 'leyka'), 'form_enctype' => 'multipart/form-data'));
        $section->add_block(new Leyka_Text_Block(array(
            'id' => 'section-intro-text',
...
<p>If you have to close this page, we are going to remember the passed steps and you may always return here.</p>', 'leyka'),
        ...
        )))->add_handler(array($this, 'handle_send_documents'))->add_to($stage);
        ...
        $section = new Leyka_Settings_Section('copy_key',  $stage->id, __('Set up your Public ID and API password', 'leyka'), array('next_label' => __('Save & continue', 'leyka')));
        $section->add_block(new Leyka_Text_Block(array(
        ...
        $section = new Leyka_Settings_Section('cp_going_live',  $stage->id, __('Going live', 'leyka'), array('next_label' => __('Send & continue', 'leyka')));
        $section->add_block(new Leyka_Text_Block(array(
            'id' => 'section-intro-text',
        ...
        )))->add_handler(array($this, 'handle_going_live'))->add_to($stage);
        ...
        $section->add_block(new Leyka_Text_Block(array(
            ...
            'keys' => array('payment_tryout_completed'),
            'rendering_type' => 'template',
            'data' => array('required' => __('Perform the payment to continue.', 'leyka'), 'is_live' => true)
        )))->add_to($stage);
        ...
        $section = new Leyka_Settings_Section('cp_final', $stage->id, __('Congratulations!', 'leyka'), array('header_classes' => 'greater',));
        $section->add_block(new Leyka_Text_Block(array(
            'id' => 'section-intro-text',
    ...
    protected function _init_navigation_data() {
        ...
        $this->_navigation_data = array(
            ...
            array('stage_id' => 'final', 'title' => __('Finish', 'leyka'), 'url' => '',),
        );
