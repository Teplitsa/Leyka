...
//            ),
        ...
        foreach(leyka_get_main_currencies_full_info() as $currency_id => $currency_defaults) {

            $currencies_options_meta = array_merge($currencies_options_meta, array(
                "currency_{$currency_id}_label" => array(
                    'type' => 'text',
                    'default' => $currency_defaults['label'],
                    'title' => sprintf(__('%s label', 'leyka'), $currency_defaults['title']),
                    'description' => sprintf(__('Please set the %s currency label here.', 'leyka'), $currency_defaults['title']),
                    'required' => true,
                    'placeholder' => sprintf(__('E.g., %s', 'leyka'), $currency_defaults['label']),
                    'length' => 6,
                ),
                "currency_{$currency_id}_min_sum" => array(
                    'type' => 'number',
                    'default' => $currency_defaults['min_amount'],
                    'title' => sprintf(__('Minimum sum available for %s', 'leyka'), $currency_defaults['title']),
                    'description' => sprintf(__('Please set minimum sum available for %s donations.', 'leyka'), $currency_defaults['title']),
                    'required' => true,
                    'placeholder' => sprintf(__('E.g., %s', 'leyka'), $currency_defaults['min_amount']),
                    'length' => 6,
                ),
                "currency_{$currency_id}_max_sum" => array(
                    'type' => 'number',
                    'default' => $currency_defaults['max_amount'],
                    'title' => sprintf(__('Maximum sum available for %s', 'leyka'), $currency_defaults['title']),
                    'description' => sprintf(__('Please set maximum sum available for %s donations.', 'leyka'), $currency_defaults['title']),
                    'required' => true,
                    'placeholder' => sprintf(__('E.g., %s', 'leyka'), $currency_defaults['max_amount']),
                    'length' => 6,
                ),
                "currency_{$currency_id}_flexible_default_amount" => array(
                    'type' => 'number',
                    'default' => $currency_defaults['flexible_default_amount'],
                    'title' => sprintf(__('Default amount of donation in %s (for «flexible» donation type)', 'leyka'), $currency_defaults['title']),
                    'description' => sprintf(__('Please, set a default amount of donation when %s selected as currency.', 'leyka'), $currency_defaults['title']),
                    'required' => true,
                    'placeholder' => sprintf(__('E.g., %s', 'leyka'), $currency_defaults['flexible_default_amount']),
                    'length' => 6,
                ),
                "currency_{$currency_id}_fixed_amounts" => array(
                    'type' => 'text',
                    'default' => $currency_defaults['fixed_amounts'],
                    'title' => sprintf(__('Possible amounts of donation in %s (for «fixed» donation type)', 'leyka'), $currency_defaults['title']),
                    'description' => sprintf(__('Please, set possible amounts of donation in %s when «fixed» donation type is selected. Only an integer non-negative values, separated with commas.', 'leyka'), $currency_defaults['title']),
                    'required' => true,
                    'placeholder' => sprintf(__('E.g., %s', 'leyka'), $currency_defaults['fixed_amounts']),
                    'length' => 25,
                ),
            ));
