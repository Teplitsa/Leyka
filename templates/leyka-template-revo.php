...
<form id="<?php echo esc_attr(leyka_pf_get_form_id($campaign->id)).'-revo-form';?>" class="leyka-inline-campaign-form leyka-revo-form" data-template="revo" action="<?php echo esc_attr(Leyka_Payment_Form::get_form_action());?>" method="post" novalidate="novalidate">

	<!-- Step 1: amount -->
    <div class="step step--amount step--active">

        <div class="step__title step__title--amount"><?php esc_html_e('Donation amount', 'leyka');?></div>
        ...
        echo wp_kses_post( $form_api->get_hidden_amount_fields() );?>
            ...
            <div class="amount__figure">
                ...
                <input type="text" class="leyka_donation_amount" name="leyka_donation_amount" value="<?php echo esc_attr( $template_data['amount_default'] );?>" autocomplete="off" placeholder="<?php echo esc_attr(apply_filters('leyka_form_free_amount_placeholder', $template_data['amount_default']));?>" data-default-value="<?php echo esc_attr( $template_data['amount_default'] );?>" data-min-value="<?php echo esc_attr( $template_data['amount_min'] );?>" data-max-value="<?php echo esc_attr( $template_data['amount_max_total'] );?>">
                <span class="curr-mark"><?php echo esc_html( $template_data['currency_label'] );?></span>

                <input type="hidden" class="leyka_donation_currency" name="leyka_donation_currency" data-currency-label="<?php echo esc_attr( $template_data['currency_label'] );?>" value="<?php echo esc_attr(leyka_options()->opt('currency_main'));?>">
            ...
            <div class="amount__icon">
                ...
                <div class="leyka_donation_amount-error field-error amount__error"><?php 
                    ...
                    printf(
                        wp_kses_post(__('Please enter valid amount <br />from %1$s to %2$s %3$s', 'leyka')),
                        esc_html($template_data['amount_min']),
                        esc_html($template_data['amount_max_total']),
                        esc_html($template_data['currency_label'])
                    );
                ?></div>
            </div>
        ...
        <?php if(leyka_is_recurring_supported()) {?>
            ...
            <a href="cards" class="leyka-js-amount"><?php esc_html_e('Support once', 'leyka');?></a>
            <a href="person" class="leyka-js-amount monthly">
                <svg class="svg-icon icon-card"><use xlink:href="#icon-card"></svg><?php esc_html_e('Support monthly', 'leyka');?>
            </a>

        <?php } else {?>
            <a href="cards" class="leyka-js-amount"><?php esc_html_e('Proceed', 'leyka');?></a>
        <?php }?>
        </div>
    </div>
    ...
    <div class="step step--cards">
        ...
        <div class="step__selection">
            <a href="amount" class="leyka-js-another-step">
                <span class="remembered-amount">#SUM#</span><span class="curr-mark"><?php echo esc_html( $template_data['currency_label'] );?></span><span class="remembered-monthly"><?php esc_html_e('monthly', 'leyka');?></span>
            </a>
        </div>

        <div class="step__title"><?php esc_html_e('Payment method', 'leyka');?></div>
    ...
    <div class="step step--static <?php echo esc_attr( $pm->full_id );?>">
        <div class="step__selection">
            <a href="amount" class="leyka-js-another-step">
                <span class="remembered-amount">#SUM#</span><span class="curr-mark"><?php echo esc_attr( $template_data['currency_label'] );?></span><span class="remembered-monthly"><?php esc_html_e('monthly', 'leyka');?></span>
            </a>
        ...
        <div class="step__border">
        	...
        	<div class="step__fields static-text">
                ...
                <div class="static__complete-donation">
                    <input class="leyka-js-complete-donation" value="<?php echo esc_attr(leyka_options()->opt_safe('revo_donation_complete_button_text'));?>">
                </div>
    ...
    <div class="step step--person">
        ...
        <div class="step__border">
            <div class="step__title"><?php esc_html_e('Whom should we thank?', 'leyka');?></div>
            <div class="step__fields donor">
                ...
                <div class="donor__textfield donor__textfield--name ">
                    <label for="<?php echo esc_attr( $field_id );?>">
                        <span class="donor__textfield-label leyka_donor_name-label">
                            <?php echo esc_attr(apply_filters('leyka_revo_donor_name_field_label', __('Your name', 'leyka'), $campaign));?>
                        </span>
                        <span class="donor__textfield-error leyka_donor_name-error">
                            <?php esc_html_e('Enter your name', 'leyka');?>
                        </span>
                    </label>
                ...
                <div class="donor__textfield donor__textfield--email">
                    <label for="<?php echo esc_attr( $field_id );?>">
                        <span class="donor__textfield-label leyka_donor_name-label"><?php esc_html_e('Your email', 'leyka');?></span>
                ...
                <div class="donor__textfield donor__textfield--comment leyka-field">
                    <label for="<?php echo esc_attr( $field_id );?>">
                        <span class="donor__textfield-label leyka_donor_comment-label"><?php
                            echo wp_kses_post(leyka_options()->opt_template('donation_comment_max_length') ? sprintf(__('Your comment (<span class="donation-comment-current-length">0</span> / <span class="donation-comment-max-length">%d</span> symbols)', 'leyka'), leyka_options()->opt_template('donation_comment_max_length')) : __('Your comment', 'leyka'));
                        ?></span>
                        <span class="donor__textfield-error leyka_donor_comment-error"><?php esc_html_e('Entered value is too long', 'leyka');?></span>
                    </label>
                    <textarea id="<?php echo esc_attr( $field_id );?>" class="leyka-donor-comment" name="leyka_donor_comment" data-max-length="<?php echo esc_attr(leyka_options()->opt_template('donation_comment_max_length'));?>"></textarea>
                </div>
                <?php }?>
                ...
                <div class="donor__submit">
                    <?php
                        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                        echo apply_filters('leyka_revo_template_final_submit', '<input type="submit" class="leyka-default-submit" value="'.leyka_options()->opt_template('donation_submit_text').'">');?>
                </div>
                ...
                <div class="donor__oferta">
                    ...
                    <?php if(leyka_options()->opt('agree_to_terms_needed')) {
                        ...
                        echo wp_kses_post(apply_filters('agree_to_terms_text_link_part', leyka_options()->opt('agree_to_terms_text_link_part')));?>
                            </a>
                        </label>
                    ...
                    <div class="donor__oferta-error leyka_agree-error leyka_agree_pd-error">
                        <?php esc_html_e('You should check out and accept the Terms to donate', 'leyka');?>
                    </div>
        ...
        <div class="step__note">
			<p><?php esc_html_e('We will send the donation success notice to this address', 'leyka');?></p>
        </div>
