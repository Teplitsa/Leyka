...
add_action('init', function(){
    if(isset($_GET['tst'])) {
        ...
        $callback_data = get_transient('leyka_tmp_callback');
        $checksum_received = empty($callback_data['checksum']) ? false : $callback_data['checksum'];

        echo '<pre>Before: '.print_r($callback_data, 1).'</pre>';
        ksort($callback_data); // 1. Sort the callback data array by keys

        // 2. Create the data string to check against a checksum:
        $data_line = '';
        foreach($callback_data as $key => $value) {

            if($key === 'checksum' || $key === 'sign_alias') {
                continue;
            }
        ...
        echo '<pre>'.print_r($data_line, 1).'</pre>';

        $tmp = trim(str_replace(array("-----BEGIN CERTIFICATE-----", "-----END CERTIFICATE-----"), '', get_option('leyka_sber_public_key')));
        $tmp = "-----BEGIN CERTIFICATE-----\n".wordwrap($tmp, 64, "\n", true)."\n-----END CERTIFICATE-----";

//        $pem_cert_content =  chunk_split(base64_encode($tmp), 64, PHP_EOL);

//        echo '<pre>HERE: '.print_r(get_option('leyka_sber_public_key'), 1).'</pre>';
//        echo '<pre>HERE: '.print_r(leyka_options()->opt('sber_api_login'), 1).'</pre>';

        $public_key_id = openssl_get_publickey(openssl_x509_read($tmp));
//        echo '<pre>'.print_r(openssl_pkey_get_details($public_key_id), 1).'</pre>';
//        echo '<pre>HERE: '.print_r(openssl_get_publickey($tmp), 1).'</pre>';
//        echo '<pre>'.print_r($cert, 1).'</pre>';

        // 3. Verifying the checksum:
        $res = openssl_verify($data_line, $checksum_received, $public_key_id, 'sha512WithRSAEncryption');

        echo '<pre>Checksum verified: '.print_r($res, 1).'</pre>';
